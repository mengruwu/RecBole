...
class BiCL4HGN(HGN):
    ...
    def __init__(self, config, dataset):
        super(BiCL4HGN, self).__init__(config, dataset)
    ...
    def calculate_loss(self, interaction):
        ...
        loss = loss + self.reg_loss(user_embedding, pos_items_emb, seq_item_embedding)

        losses = [loss]
        if self.cl_type in ['un', 'rs', 'su', 'all', 'rs_su']:
            un_aug_seq_output = self.forward(seq_item, user)

        if self.cl_type in ['su', 'rs_su_x', 'rs_su', 'all']:
            aug_item_seq = interaction['aug']
            su_aug_seq_output = self.forward(aug_item_seq, user)
        
        if self.cl_type in ['rs', 'rs_su_x', 'rs_su', 'all']:
            aug_item_seq_rev = interaction['aug_rev']
            su_aug_seq_rev_output = self.forward(aug_item_seq_rev, user)
